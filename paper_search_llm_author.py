...
def main(
    ...
    for dialog, result in zip(dialogs, results):
        ...
        #)
        #open rev_author_dict.json
        with open(rev_author_dict_path, 'r') as f:
            rev_author_dict = json.load(f)
        ...
        try:
            if len(response['matches']) == 0:
                ...
                author = phrase.split("by ")[1].split(". Just")[0]
                author_id = rev_author_dict[author]
                response = requests.get("https://api.semanticscholar.org/graph/v1/author/" + author_id + "/papers?fields=title,authors,year&limit=5", headers={'x-api-key':key}).json()
                recom_paper = {}
                ...
                print("Here are some recommendations by the same Author: ")
                print("\n==================================\n")
                ...
                for paper in response['matches']:
                    if paper['title'].lower() == new_title.lower():
                        ...
                        paper_id = paper['id']
                        response = requests.get("https://api.semanticscholar.org/graph/v1/paper/" + paper_id +"?fields=title,authors,year", headers={'x-api-key':key}).json()
                        original_authors_ids = []
                        original_authors = []
                        for author in response['authors']:
                            original_authors_ids.append(int(author['authorId']))
                            original_authors.append(author['name'])
                        original_year = response['year']
                        break
                author = phrase.split("by ")[1].split(". Just")[0]
                orig_author_id = int(rev_author_dict[author])
                if orig_author_id not in original_authors_ids:
                    print("Paper Verified by Semantic Scholar")
                    print("==============But=================")
                    print("Author Not Matching")
                    with open('count/paper_count_author_wrong.txt', 'r') as f:
                        count = int(f.read())
                    count += 1
                    with open('count/paper_count_author_wrong.txt', 'w') as f:
                        f.write(str(count))
                else:
                    print("Paper Verified by Semantic Scholar")
                    ...
                    with open('count/paper_count_author_right.txt', 'r') as f:
                        count = int(f.read())
                    count += 1
                    with open('count/paper_count_author_right.txt', 'w') as f:
                        f.write(str(count))
                

                if original_year != year:
                    with open('count/paper_count_year_wrong.txt', 'r') as f:
                        count = int(f.read())
                    count += 1
                    with open('count/paper_count_year_wrong.txt', 'w') as f:
                        f.write(str(count))
                else:
                    with open('count/paper_count_year_right.txt', 'r') as f:
                        count = int(f.read())
                    count += 1
