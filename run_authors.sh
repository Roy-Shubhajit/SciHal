...
COMMAND="torchrun --nproc_per_node 1 paper_search_llm_author.py --ckpt_dir llama-2-7b-chat/ --tokenizer_path tokenizer.model --max_seq_len 512 --max_batch_size 6"

# Parse additional arguments
while [ "$#" -gt 1 ]; do
    case "$2" in
        --rev_author_dict_path)
            REVERSE_AUTHOR_DICT_PATH="$3"
            COMMAND+=" --rev_author_dict_path $REVERSE_AUTHOR_DICT_PATH"
            shift 2
            ;;
        --key)
            KEY_VALUE="$3"
